...
class LoginModel extends DatabaseModel {
  ...
  protected function getLoginAttempts(string $id_number) {
    $idNumber = $this->escape($id_number);
    $query = "SELECT login_attempts FROM edocs_users WHERE id_number = '$idNumber'";
  ...
  protected function resetLoginAttempts(string $id_number) {
    $idNumber = $this->escape($id_number);
    $query = "UPDATE edocs_users SET login_attempts = 0, lockout_time = null WHERE id_number = '$idNumber'";
    mysqli_query($this->connect(), $query);
  }

  // protected function getRoles(string $id_number) {
  //   $idNumber = $this->escape($id_number);
  //   $query = "SELECT u.*, d.dcode, d.dname, p.pcode, p.pname, r.role as rs_role, q.user_role, ex.role as ex_role, pl.role as pl_role
  //             FROM edocs_users u
  ...
  //             LEFT JOIN extension_roles ex ON ex.user_id = u.id
  //             LEFT JOIN planning_roles pl ON pl.user_id = u.id
